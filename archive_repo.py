...
if not USER_EMAIL:
    raise EnvironmentError("USER_EMAIL is not set in the environment variables")
...
if __name__ == '__main__':
    ...
    # Allows user to enter a repo through the command line
    repo_path = input ("\nPlease enter the repo path:")

    print("\nThe url being searched is: ",repo_path)

    # If the link is https strip from com/ and if ssh stip from com:
    ...
    # Passes the repo path to the get the id for a specific repo
    project_id = get_project_id(repo_path)
    print("\nThe project ID is:", project_id)

    # Check if the project_id is empty or none
    if project_id:
        ...
        try:
            ...
            glab_username = resolve_gitlab_username(USER_EMAIL, gitlab_usernames)
            ...
            if roles and glab_username in roles: # Checks if roles is not empty and if the user is in the roles
                print(f"\nUsername: {glab_username}, Access Level: {permisson_value}")

                if permisson_value >= 30:
                    print("\n\033[92mUser has required access to archive\033[0m")
                    ...
                    try:
                        ...
                        if response.status_code == 201:
                            print("\n\033[92mThe repo has been archived successfully\033[0m")
                        else:
                            print(f'\nFailed to archive project: {response.status_code} - {response.text}')
                    except requests.exceptions.RequestException as e:
                        print(f'\n\n\033[91mFailed to archive project: {e}\033[0m')
                        raise Exception
                    
                
                elif permisson_value < 30 :# redundant check as if not 40 will always be less than 31, better for extra security ?
                    print("\n\033[91mUser does not have the required access to archive\033[0m")
                    raise Exception
            else:
                print(f"\n\033[91mThe user: {glab_username} was not found\033[0m")
                raise Exception
        except Exception as e:
            print(f"\n\033[91mError: {e}\033[0m")
            raise Exception
    else:
        print("\n\033[91mFailed to retrieve project information.\33[0m")
        raise Exception
